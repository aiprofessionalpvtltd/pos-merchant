@extends('admin.layouts.app')

@section('content')
    <div class="page-header page-header-light">
        <div class="page-header-content header-elements-md-inline">
            <div class="page-title d-flex">
                <h4><span class="font-weight-semibold">{{ $title }}</span></h4>
            </div>
        </div>
    </div>

    <div class="content">
        <div class="card">
            <form action="{{ route('admin.subscription-plans.update', $plan) }}" method="post">
                @csrf
                @method('PUT')
                <div class="card-body">
                    @if($usage > 0)
                        <div class="alert alert-info">
                            {{ $usage }} merchant subscription(s) or open payment(s) use this plan. A new price applies
                            to the next payment; merchants who already paid keep their current period.
                            A feature you remove is taken away from them straight away.
                        </div>
                    @endif

                    @include('admin.subscription_plans._form')

                    <div class="mt-4">
                        <a href="{{ route('admin.subscription-plans.index') }}" class="btn btn-light">Cancel</a>
                        <button type="submit" class="btn btn-outline-primary">Save plan</button>
                    </div>
                </div>
            </form>
        </div>
    </div>
@endsection
